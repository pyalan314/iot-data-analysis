...
anomaly1_start_idx = int(len(date_range) * 0.50)
anomaly1_length = 36
anomaly1_end_idx = min(anomaly1_start_idx + anomaly1_length, len(date_range))
anomaly1_magnitude = np.linspace(0, 5, anomaly1_end_idx - anomaly1_start_idx)
temperature[anomaly1_start_idx:anomaly1_end_idx] += anomaly1_magnitude

anomaly2_start_idx = int(len(date_range) * 0.85)
anomaly2_length = 48
anomaly2_end_idx = min(anomaly2_start_idx + anomaly2_length, len(date_range))
anomaly2_magnitude = np.linspace(0, 8, anomaly2_end_idx - anomaly2_start_idx)
...
print(f"\nAnomalies injected:")
print(f"  Anomaly 1 (middle): index {anomaly1_start_idx} ({df['timestamp'].iloc[anomaly1_start_idx]}) - moderate surge")
print(f"  Anomaly 2 (end): index {anomaly2_start_idx} ({df['timestamp'].iloc[anomaly2_start_idx]}) - severe surge")

plt.figure(figsize=(15, 6))
plt.plot(df['timestamp'], df['temperature'], linewidth=0.8, alpha=0.8)
plt.axvline(x=df['timestamp'].iloc[anomaly1_start_idx], color='orange', linestyle='--', 
            label='Anomaly 1 (Middle)', alpha=0.7)
plt.axvline(x=df['timestamp'].iloc[anomaly2_start_idx], color='red', linestyle='--', 
            label='Anomaly 2 (End)', alpha=0.7)
